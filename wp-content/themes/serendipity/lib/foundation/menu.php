...
add_theme_support(
        'genesis-menus',
        array(
                'topbar-off-canvas' => 'Topbar Off Canvas',
                'topbar-tablet-and-up' => 'Topbar Tablet And Up',
        )
);
 ...
 * Mobile off-canvas
 */
if ( ! function_exists( 'ygf_topbar_off_canvas' ) ) {
  function ygf_topbar_off_canvas() {
      wp_nav_menu(array(
          'container' => false,                           // remove nav container
          'container_class' => '',                        // class of container
          'items_wrap' => '<ul id="%1$s" class="%2$s" data-accordion-menu><li><span id="menu-title">Navigation</span><button class="close-button" aria-label="Close menu" type="button" data-close><span aria-hidden="true">&times;</span></button></li>%3$s</ul>',
          'menu_class' => 'vertical menu',                                   // menu name
          'theme_location' => 'topbar-off-canvas',        // where it's located in the theme
          'before' => '',                                 // before each link <a>
          'after' => '',                                  // after each link </a>
          'link_before' => '',                            // before each link text
          'link_after' => '',                             // after each link text
...
if ( ! function_exists( 'ygf_topbar_off_canvas_display' ) ) {
  function ygf_topbar_off_canvas_display() {
      if ( has_nav_menu('topbar-off-canvas') ) {
          echo '<aside class="off-canvas position-left" id="offCanvas" data-off-canvas>';
              ygf_topbar_off_canvas();
          echo '</aside>';
      }
    }
    add_action( 'genesis_before', 'ygf_topbar_off_canvas_display');
